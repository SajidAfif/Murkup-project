...
router.get('/', async (req, res) => {
    try {
        ...
        properties = properties.map((p) => {
            ...
            obj.images = (obj.images || []).map((img) => (img.startsWith('http') ? img : `${base}/${img}`));
            return obj;
        });
        res.json(properties);
    }
...
router.patch('/:id', auth, upload.array('images', 6), async (req, res) => {
    try {
        ...
        if (rentalType !== undefined)
            prop.rentalType = rentalType;
        if (price !== undefined)
            prop.price = Number(price);
        if (rooms !== undefined)
            prop.rooms = rooms ? Number(rooms) : undefined;
        if (bathrooms !== undefined)
            prop.bathrooms = bathrooms ? Number(bathrooms) : undefined;
        if (furnishing !== undefined)
            prop.furnishing = furnishing;
        if (sqft !== undefined)
            prop.sqft = sqft ? Number(sqft) : undefined;
        if (amenities !== undefined)
            prop.amenities = typeof amenities === 'string' ? JSON.parse(amenities) : amenities;
        // Parse location if sent as JSON string
        if (location) {
            try {
        ...
        if (deletedImages) {
            ...
            try {
                if (typeof deletedImages === 'string')
                    del = JSON.parse(deletedImages);
            }
            catch (err) {
                // ignore
            }
            if (Array.isArray(del)) {
                prop.images = (prop.images || []).filter((img) => {
                    if (del.includes(img)) {
        ...
        const addedFiles = req.files;
        const added = addedFiles?.map((f) => f.path) || [];
        if (added.length > 0)
            prop.images = (prop.images || []).concat(added);
        await prop.save();
        const base = `${req.protocol}://${req.get('host')}`;
...
router.post('/:id/like', auth, async (req, res) => {
    try {
        ...
        const idx = prop.likes?.findIndex((u) => u.toString() === userId);
        let liked = false;
        if (idx >= 0) {
            // remove
            prop.likes.splice(idx, 1);
            liked = false;
        }
        else {
            prop.likes = prop.likes || [];
            prop.likes.push(userId);
            liked = true;
        }
        await prop.save();
        res.json({ liked, likesCount: prop.likes.length });
    }
    catch (error) {
